...
/**
 * @author Doston Bokhodirov, Sat 2:45 PM. 12/25/2021
 */
@NoArgsConstructor(access = AccessLevel.PRIVATE)
public class DownloadedBookProcessor {
    private static final DownloadedBookProcessor instance = new DownloadedBookProcessor();
    ...
    public void process(Message message) {
        ...
        ArrayList<Book> books = bookRepository.getDownloadedBooks(chatId, State.getLimitState(chatId), offset.getSearchOffset(chatId));
        SendMessage sendMessage;
        if (books.size() == 0) {
            sendMessage = new SendMessage(chatId, LangConfig.get(chatId, "no.downloaded.book"));
    ...
    public static DownloadedBookProcessor getInstance() {
        return instance;
    }
}
